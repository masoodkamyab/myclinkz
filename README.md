...
- 🎯 Entry points to execute your program using `python -m <myclinkz>` or `$ myclinkz` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/masoodkamyab/myclinkz/branch/main/graph/badge.svg?token=myclinkz_token_here)](https://codecov.io/gh/masoodkamyab/myclinkz)
[![CI](https://github.com/masoodkamyab/myclinkz/actions/workflows/main.yml/badge.svg)](https://github.com/masoodkamyab/myclinkz/actions/workflows/main.yml)

Awesome myclinkz created by masoodkamyab

## Install

from source
```bash
git clone https://github.com/masoodkamyab/myclinkz myclinkz
cd myclinkz
make install
```
...
```bash
pip install myclinkz
```

## Executing

```bash
$ myclinkz run --port 8080
```

or

```bash
python -m myclinkz run --port 8080
```

or

```bash
$ uvicorn myclinkz:app
```
...
```bash
❯ myclinkz --help
Usage: myclinkz [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ myclinkz create-user --help
Usage: myclinkz create-user [OPTIONS] USERNAME PASSWORD
...
```bash
myclinkz create-user admin admin --superuser
```
...
Run with `myclinkz run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `myclinkz create-user`.
...
-----------------------------------------------------
myclinkz/__init__.py              4      0   100%
myclinkz/app.py                  16      1    94%
myclinkz/cli.py                  21      0   100%
myclinkz/config.py                5      0   100%
myclinkz/db.py                   10      0   100%
myclinkz/models/__init__.py       0      0   100%
myclinkz/models/content.py       47      1    98%
myclinkz/routes/__init__.py      11      0   100%
myclinkz/routes/content.py       52     25    52%
myclinkz/routes/security.py      15      1    93%
myclinkz/routes/user.py          52     26    50%
myclinkz/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in myclinkz/default.toml.
